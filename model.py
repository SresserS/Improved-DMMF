...
class SelfAttentionModel(nn.Module):
    def __init__(self, num_features):
        ...
        self.num_features = num_features
        num_heads = num_features // 2
        self.self_attention = nn.MultiheadAttention(embed_dim=num_features, num_heads=num_heads)
        # self.projection = nn.Linear(num_features, 12)
    ...
    def forward(self, x):
        ...
        # x = self.projection(attention_output)
        x = self.activation(attention_output)
        x = x.permute(0, 2, 1).view(batch_size, self.num_features, height, width)
...
class DM2FNet_woPhy_selfatt(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        super(DM2FNet_woPhy_selfatt, self).__init__()
        self.num_features = num_features
        ...
        self.attmodel = SelfAttentionModel(num_features=num_features)
    ...
    def forward(self, x0):
        ...
        x_p3 = torch.exp(-log_x_p3_0 + F.upsample(self.p3_1(f), size=x0.size()[2:], mode='bilinear')).clamp(min=0,
                                                                                                            max=1)
        # print(f.shape)
        attention_fusion = F.upsample(self.attmodel(f), size=x0.size()[2:], mode='bilinear')
...
from modeling_unet import  UNet_norm
from unet.unet_model import UNet

class DM2FNet_woPhy_unet(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        super(DM2FNet_woPhy_unet, self).__init__()
        self.num_features = num_features
    ...
    def forward(self, x0):
        ...
        # unet_decoded = self.unet(torch.cat([x_fusion, x], dim=1))
        
        if self.training:
            return unet_output, x_fusion, x_p0, x_p1, x_p2, x_p3
        else:
            return unet_decoded


from timm import create_model
class convnext_base(nn.Module): #torch.Size([512, 14, 14])
    def __init__(self):
        ...
        pretrained_cfg = create_model(model_name).default_cfg
        pretrained_cfg['file'] = "/path/convnext_base_22k_224.pth"
        self.model = create_model(model_name,pretrained=True, pretrained_cfg=pretrained_cfg,features_only=True)

    def forward(self,x):
        # x = F.interpolate(x, size=(448, 448), mode='bilinear', align_corners=False)
        feat = self.model(x)
        # feat = F.interpolate(feat, size=(28, 28), mode='bilinear', align_corners=False)
        # out = feat[2]
        return feat
...
class DM2FNet_woPhy_my(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        ...
        self.down0 = nn.Sequential(
            nn.Conv2d(128, num_features, kernel_size=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU()
        )
        self.down1 = nn.Sequential(
        ...
        self.fuse3_selfatt = SelfAttentionModel(num_features * 2)
        self.fuse2_selfatt = SelfAttentionModel(num_features * 2)
        self.fuse1_selfatt = SelfAttentionModel(num_features * 2)
        self.fuse0_selfatt = SelfAttentionModel(num_features * 2)
    ...
    def forward(self, x0):
        ...
        down4 = F.upsample(down4, size=down3.size()[2:], mode='bilinear')
        fu3 = self.fuse3_selfatt(torch.cat((down4, down3), 1))
        fuse3_attention = self.fuse3_attention(fu3)
        f = down4 + self.fuse3(torch.cat((down4, fuse3_attention * down3), 1))

        f = F.upsample(f, size=down2.size()[2:], mode='bilinear')
        fu2 = self.fuse2_selfatt(torch.cat((f, down2), 1))
        fuse2_attention = self.fuse2_attention(fu2)
        f = f + self.fuse2(torch.cat((f, fuse2_attention * down2), 1))

        f = F.upsample(f, size=down1.size()[2:], mode='bilinear')
        fu1 = self.fuse1_selfatt(torch.cat((f, down1), 1))
        fuse1_attention = self.fuse1_attention(fu1)
        f = f + self.fuse1(torch.cat((f, fuse1_attention * down1), 1))

        f = F.upsample(f, size=down0.size()[2:], mode='bilinear')
        fu0 = self.fuse0_selfatt(torch.cat((f, down0), 1))
        fuse0_attention = self.fuse0_attention(fu0)
        f = f + self.fuse0(torch.cat((f, fuse0_attention * down0), 1))
        ...
        x_p3 = torch.exp(-log_x_p3_0 + F.upsample(self.p3_1(f), size=x0.size()[2:], mode='bilinear')).clamp(min=0,
                                                                                                            max=1)
        # print(f.shape)
        fu_self = self.attmodel(f)
        attention_fusion = F.upsample(self.attentional_fusion(fu_self), size=x0.size()[2:], mode='bilinear')
        # print(attention_fusion.shape)
        x_fusion = torch.cat((torch.sum(F.softmax(attention_fusion[:, : 4, :, :], 1) * torch.stack(
            (x_p0[:, 0, :, :], x_p1[:, 0, :, :], x_p2[:, 0, :, :], x_p3[:, 0, :, :]), 1), 1, True),
                              torch.sum(F.softmax(attention_fusion[:, 4: 8, :, :], 1) * torch.stack((x_p0[:, 1, :, :],
...
class DM2FNet_woPhy_my_resnext(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        super(DM2FNet_woPhy_my_resnext, self).__init__()
        self.num_features = num_features
...
class DM2FNet_my(Base):
    def __init__(self, num_features=128, arch='resnext101_32x8d'):
        super(DM2FNet_my, self).__init__()
        self.num_features = num_features
        ...
        self.fuse4_selfatt = SelfAttentionModel(num_features * 4)
        self.fuse3_selfatt = SelfAttentionModel(num_features * 4)
        self.fuse2_selfatt = SelfAttentionModel(num_features * 4)
        self.fuse1_selfatt = SelfAttentionModel(num_features * 4)
        ...
        self.attmodel = SelfAttentionModel(num_features=num_features * 4)
    ...
    def forward(self, x0, x0_hd=None):
        ...
        att1_concat = self.fuse1_selfatt(concat)
        attention1 = self.attention1(att1_concat)
        attention1 = F.softmax(attention1.view(n, 4, c, h, w), 1)
        f1 = down1 * attention1[:, 0, :, :, :] + down2 * attention1[:, 1, :, :, :] + \
             down3 * attention1[:, 2, :, :, :] + down4 * attention1[:, 3, :, :, :]
        f1 = self.refine(f1) + f1

        att2_concat = self.fuse2_selfatt(concat)
        attention2 = self.attention2(att2_concat)
        attention2 = F.softmax(attention2.view(n, 4, c, h, w), 1)
        f2 = down1 * attention2[:, 0, :, :, :] + down2 * attention2[:, 1, :, :, :] + \
             down3 * attention2[:, 2, :, :, :] + down4 * attention2[:, 3, :, :, :]
        f2 = self.refine(f2) + f2

        att3_concat = self.fuse3_selfatt(concat)
        attention3 = self.attention3(att3_concat)
        attention3 = F.softmax(attention3.view(n, 4, c, h, w), 1)
        f3 = down1 * attention3[:, 0, :, :, :] + down2 * attention3[:, 1, :, :, :] + \
             down3 * attention3[:, 2, :, :, :] + down4 * attention3[:, 3, :, :, :]
        f3 = self.refine(f3) + f3

        att4_concat = self.fuse4_selfatt(concat)
        attention4 = self.attention4(att4_concat)
        attention4 = F.softmax(attention4.view(n, 4, c, h, w), 1)
        f4 = down1 * attention4[:, 0, :, :, :] + down2 * attention4[:, 1, :, :, :] + \
             down3 * attention4[:, 2, :, :, :] + down4 * attention4[:, 3, :, :, :]
        f4 = self.refine(f4) + f4
        ...
        att_self_fu = self.attmodel(concat)
        attention_fusion = F.upsample(self.attention_fusion(att_self_fu), size=x0.size()[2:], mode='bilinear')
        x_f0 = torch.sum(F.softmax(attention_fusion[:, :5, :, :], 1) *
                         torch.stack((x_phy[:, 0, :, :], x_j1[:, 0, :, :], x_j2[:, 0, :, :],
                                      x_j3[:, 0, :, :], x_j4[:, 0, :, :]), 1), 1, True)
        x_f1 = torch.sum(F.softmax(attention_fusion[:, 5: 10, :, :], 1) *
