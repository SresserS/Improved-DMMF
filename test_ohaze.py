...
from model import DM2FNet, DM2FNet_woPhy, DM2FNet_woPhy_my
...
args = {
    ...
    'snapshot': 'iter_20000_loss_0.05888_lr_0.000000',
}
...
def main():
    with torch.no_grad():
        ...
        for name, root in to_test.items():
            ...
            print(f"[{name}] L1: {loss_record.avg:.6f}, PSNR: {np.mean(psnrs):.6f}, SSIM: {np.mean(ssims):.6f}, MSE: {np.mean(mses):.6f}, CIEDE: {np.mean(ciedes):.6f}")
